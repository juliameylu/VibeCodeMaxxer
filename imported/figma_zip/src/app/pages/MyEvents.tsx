...
import { Heart, MapPin, Users, Trash2, Share2, ChevronRight } from "lucide-react";
    ...
    <div className="min-h-[100dvh] bg-transparent text-white pb-24">
      ...
      <div className="px-5 pb-2">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-2xl font-black text-white tracking-tight uppercase">MY EVENTS</h1>
          <Link to="/groups">
            <button className="flex items-center gap-1.5 text-[10px] font-black text-[#8BC34A] bg-[#8BC34A]/10 px-3 py-2 rounded-full active:scale-95 transition-transform uppercase tracking-wider">
              <Users size={14} /> GROUPS
            </button>
          </Link>
        </div>
        <p className="text-[10px] text-white/25 font-bold uppercase tracking-wider">EVENTS YOU'VE CONFIRMED OR SAVED</p>
      </div>
      ...
      <div className="px-5 py-3 flex gap-2">
        {([
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-full text-[10px] font-black tracking-wider transition-all ${
              filter === tab.key
          ...
          <AnimatePresence>
            {filtered.map(({ id, status, place }) => (
              ...
              >
                ...
                <div className="flex border-t border-white/5 text-[10px] font-black">
                  {status === "maybe" ? (
                    <button
                      onClick={() => changeStatus(id, "confirmed")}
                      className="flex-1 py-2 text-[#8BC34A] flex items-center justify-center gap-1 active:bg-white/5 uppercase tracking-wider"
                    >
                    ...
                    <button
                      ...
                      className="flex-1 py-2 text-[#F2E8CF] flex items-center justify-center gap-1 active:bg-white/5 uppercase tracking-wider"
                    >
                      MOVE TO MAYBE
                    </button>
                  )}
                  ...
                  <button
                    ...
                    className="flex-1 py-2 text-white/30 flex items-center justify-center gap-1 active:bg-white/5 uppercase tracking-wider"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 py-2 text-white/15 flex items-center justify-center gap-1 hover:text-red-400 active:bg-white/5 uppercase tracking-wider"
                  >
