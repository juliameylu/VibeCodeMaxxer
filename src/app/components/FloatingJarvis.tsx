...
const HIDDEN_PATHS = ["/", "/landing", "/signin", "/preferences", "/jarvis", "/tutorial"];
...
function hasCompletedPreferences(): boolean {
  try {
    const raw = localStorage.getItem("polyjarvis_prefs");
    if (!raw) return false;
...
export function FloatingJarvis() {
  ...
  const prefsReady = hasCompletedPreferences();
  ...
  return (
    <>
      ...
      {!open && (
        <div
          className="fixed z-[95] flex flex-col items-center gap-2 right-4"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed right-3 left-3 z-[100] bg-[#0d1208]/90 backdrop-blur-2xl border border-[#F2E8CF]/15 shadow-2xl shadow-black/50 max-h-[60vh] flex flex-col overflow-hidden"
            style={{
              bottom: "calc(var(--bottom-nav-height, 88px) + 12px)",
              borderRadius: 16,
