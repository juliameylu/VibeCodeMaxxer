...
function toPriceParam(price: string): string | null {
  if (!price || price === "all") return null;
  const mapped = Math.max(1, Math.min(4, price.length));
  return String(mapped);
}

function toSortParam(sortBy: string): "best_match" | "rating" | "distance" {
...
router.get("/api/places", async (req, res) => {
  ...
  const location = String(process.env.YELP_LOCATION || "San Luis Obispo, CA");
  ...
  try {
    ...
    const items = businesses.map((biz: any) => ({
      id: String(biz?.id || ""),
      ...
      categories: Array.isArray(biz?.categories)
        ? biz.categories.map((row: any) => ({
            alias: String(row?.alias || ""),
            title: String(row?.title || ""),
          }))
        : [],
      transactions: Array.isArray(biz?.transactions)
        ? biz.transactions.map((row: any) => String(row || ""))
        : [],
    }));
    ...
    res.status(502).json({
      error:
        error instanceof Error
          ? `Yelp upstream request failed: ${error.message}`
          : "Yelp upstream request failed.",
    });
  }
});
